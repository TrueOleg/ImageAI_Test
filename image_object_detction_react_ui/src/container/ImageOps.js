        ...
        this.setState({ active_type: type }, () => {
            ...
            if (!this.state.image_object_details[this.state.active_type]) {
                api("detect_image_objects", {
                    ...
                    const filtered_data = response.getReader();
                    console.log('filterfiltered_data', filtered_data);
                    filtered_data.read().then(({ done, value }) => {
                        console.log('value', value);
                        let base64String = btoa(String.fromCharCode(...new Uint8Array(value)));
                        console.log('base64', base64String);
                        this.setState({ encodedData: base64String });
                    })
                });
            }
        });
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        {this.state.encodedData &&
                            <img src={"data:image/jpg;base64, " + this.state.encodedData} alt="" height="500px" />
                        }
                    </Grid>
                    <Grid item xs={12}>
                        <Card>
